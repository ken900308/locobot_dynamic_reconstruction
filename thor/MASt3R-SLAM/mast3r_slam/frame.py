...
class SharedKeyframes:
    def __init__(self, manager, h, w, buffer=512, dtype=torch.float32, device="cuda"):
        ...
        # [Custom Modification for Unity IBGR] 
        # 原本 MASt3R 依賴 is_dirty 標記每個 Keyframe 的新增，
        # 但為了讓 Unity 知道「後端 PGO 真的算完並更新了一次完整的系統級地圖軌跡」，
        # 我們新增了這個 pgo_flag，只有當整批軌跡被覆寫時才會標註為 1。
        self.pgo_flag = manager.Value("i", 0)
    ...
    def update_T_WCs(self, T_WCs, idx) -> None:
        with self.lock:
            ...
            # [Custom Modification for Unity IBGR]
            # 當這個函數被 global_opt.py (後端優化器) 呼叫時，代表 PGO 算出並覆寫了最新的軌跡矩陣。
            # 我們在此亮起信號燈，通知主程式可以把最新的全量地圖打給 Unity 的 Buffer B 了。
            self.pgo_flag.value = 1

    # [Custom Modification for Unity IBGR]
    # 提供給 mast3r_slam_visual_IGBR.py 主迴圈的查詢接口。
    # 查詢後即刻重置為 0，確保每一次 PGO 計算只會觸發一次全量點雲發送，避免塞爆 IPC Socket。
    def check_and_clear_pgo_flag(self) -> bool:
        """檢查 PGO 是否已更新，若有則重置並回傳 True"""
        with self.lock:
            if self.pgo_flag.value == 1:
                self.pgo_flag.value = 0
                return True
            return False
