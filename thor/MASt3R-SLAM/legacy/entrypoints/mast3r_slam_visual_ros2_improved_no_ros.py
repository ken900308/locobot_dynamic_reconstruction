    ...
    def publish_keyframe_pose_updates(self):
        ...
        try:
            with self.keyframes_lock:
                ...
                # [HYBRID PGO FIX] Instead of only sending 'dirty' frames,
                # we MUST send ALL keyframes so Unity can apply Delta-T corrections
                # to the ENTIRE global map perfectly (the jelly effect).
                # If we only send dirty frames, only the new points move while the old map stays rigid.
                kf_ids = []
                scales = []
                poses = []
                ...
                for idx in range(num_kfs):
                    ...
                    # [FIX] Unity requires exactly normalized quaternions
                    ...
                    # --------------------------------------------------------------------------------
                    # MASt3R is OpenGL World: X Right, Y Up, Z Backward (Right-Handed)
                    # Unity is Left-Handed: X Right, Y Up, Z Forward (Left-Handed)
                    #
                    # Conversion Rule (Z-Flip):
                    # Point/Position: (x_u, y_u, z_u) = (x_r, y_r, -z_r)
                    # Quaternion: Flip X and Y components (or equivalently Z and W)
                    # (qx_u, qy_u, qz_u, qw_u) = (-qx_r, -qy_r, qz_r, qw_r)
                    # --------------------------------------------------------------------------------
                    
                    tx_u, ty_u, tz_u = tx, ty, -tz
                    qx_u, qy_u, qz_u, qw_u = -qx, -qy, qz, qw
                    
                    # Store details for dict conversion
                    kf_ids.append(str(keyframe.frame_id))
                    scales.append(f"{scale:.6f}")
                    poses.append({
                        "position": {"x": float(tx_u), "y": float(ty_u), "z": float(tz_u)},
                        "orientation": {"x": float(qx_u), "y": float(qy_u), "z": float(qz_u), "w": float(qw_u)}
                    })
            
            if len(poses) == 0:
                return
            
            frame_id_str = f"kf_ids:{','.join(kf_ids)}|scales:{','.join(scales)}"
            
            if self.ros_enabled:
                # In ROS-mode, emit PoseArray (Not fully implemented here since ROS_AVAILABLE=False by default anyway)
                pass 
            else:
                if self.ipc_pointcloud_sender is not None:
                    timestamp_ns = int(time.time() * 1e9)
                    ...
                    ok = self.ipc_pointcloud_sender.send_posearray(timestamp_ns, posearray_payload)
                    if ok:
                        self.get_logger().info(f"🔄 IPC pose updates sent for {len(poses)} keyframes: {kf_ids}")
