...
#IMAGE_TOPIC=${IMAGE_TOPIC:-/camera_top/camera_top/color/image_raw/compressed}
IMAGE_TOPIC=${IMAGE_TOPIC:-/camera_top/camera_top/color/image_raw/compressed}
CAMERA_INFO_TOPIC=${CAMERA_INFO_TOPIC:-/camera_top/camera_top/color/camera_info}
DEVICE=${DEVICE:-cuda:0}

# Rosbridge 參數（用於 WiFi 不穩定環境）
USE_ROSBRIDGE=${USE_ROSBRIDGE:-false}
ROSBRIDGE_HOST=${ROSBRIDGE_HOST:-192.168.0.212}
...
show_help() {
    ...
    echo "  --rosbridge             Enable rosbridge (TCP WebSocket) for unstable WiFi"
    echo "  --rosbridge-host HOST   Rosbridge server IP (default: 192.168.0.212)"
    echo "  --rosbridge-port PORT   Rosbridge server port (default: 9090)"
    echo ""
    ...
    echo ""
    echo "  # Run with rosbridge for unstable WiFi"
    echo "  $0 --rosbridge --rosbridge-host 192.168.0.212"
}
...
if [ "$USE_ROSBRIDGE" = "true" ]; then
    echo "  • Transport: rosbridge (TCP WebSocket)"
    echo "  • Rosbridge server: $ROSBRIDGE_HOST:$ROSBRIDGE_PORT"
    echo "  • Image topic: ${IMAGE_TOPIC}/compressed (JPEG via rosbridge)"
else
    echo "  • Transport: ROS2 native (UDP DDS)"
    echo "  • Image topic: $IMAGE_TOPIC (raw)"
fi
