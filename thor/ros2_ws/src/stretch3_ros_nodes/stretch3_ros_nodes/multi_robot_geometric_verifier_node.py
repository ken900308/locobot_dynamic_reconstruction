...
class MultiRobotGeometricVerifierNode(Node):
    ...
    def on_verification_job(self, msg: String) -> None:
        ...
        if not self.try_process_job(job):
            ...
            self.publish_summary(job, "waiting_for_inputs", "waiting for local clouds and keyframe images", False)
    ...
    def try_process_job(self, job: GeometricVerificationJob) -> bool:
        ...
        reason = (
            f"verified Sim(3): matches={estimate.match_count}, correspondences={estimate.correspondence_count}, "
            f"inliers={estimate.sim3.inlier_count}, rmse={estimate.sim3.rmse:.4f} m"
        )
